...
async def get_Elspot_data(area: str, start_time: datetime, end_time: datetime) -> dict:
    """
    Retrieves Elspot price data from Nordpool for a specified area and time range.

    Args:
        area (str): The area code for which to fetch prices.
    ...
    try:
        ...
        logging.info(f"Querying Nordpool API for {area} from {start_time} to {end_time}")
        ...
        fetch_func = partial(prices_spot.hourly, areas=[area], end_date=end_time.date())
        prices_data = await loop.run_in_executor(None, fetch_func)

        # Process the data
        data = {}
        for day_data in prices_data['areas'][area]['values']:
        ...
        if data:
            ...
            logging.info(f"Nordpool Elspot price for {area} from: {start_time} to {end_time}\n"
        ...
        logging.error(f"Error details: area={area}, start_time={start_time}, end_time={end_time}")
        return {}
...
async def main():
    ...
    nordpool_data = await get_Elspot_data(area='NL', start_time=current_time, end_time=tomorrow_midnight)
