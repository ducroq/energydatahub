...
- **Pipeline**: Active development. 7 PRs merged 2026-06-06/07 (#10, #16, #18, #19, #20, #22, #23) + 15 direct-to-main commits on 2026-06-07 closing 6 issues end-to-end and addressing three reviewer batteries (tier-1 sweep 3dfc7fb → gas_storage hotfix 6eab742 → tier-2 sweep a74f662 → #27 schema-drift tripwire 7c0de64 → #25 TenneT structural fix 7673750 → #3 hydro collector 935c483 → reviewer bundle A 39a94d6 → reviewer bundle B 87566e7 → #3 hydro wire-in + multi-model review fixes c40a53b, plus memory/doc refreshes). Daily 16:00 UTC cron + workflow_dispatch on demand.
- **Active collectors**: 23 datasets reaching the published file set <!-- verify: python -c "import json; print(len(json.load(open('data/_shape_signatures.json'))['feeds']))" -->. `grid_imbalance` now soft-gated (warning, not silent-absent) per #25. `nordic_hydro` added 2026-06-07 (#3, c40a53b) — weekly cadence, NO+SE, leading indicator for NL import prices via NorNed. Completeness tripwire warns on missing expected files. Schema-drift tripwire (`scripts/detect_schema_drift.py`) runs in `--warn-only` mode initially; flip to fail-mode after ~2 weeks of clean runs (target ~2026-06-21).
...
- **Open issues**: 6 total <!-- verify: gh issue list --state open --json number | jq length -->. Closed 2026-06-07: #3 (Nordic hydro — collector + wire-in + reviewer-battery fixes in c40a53b), #4 (quick-win), #13, #14, #24, #26 (tier-1 sweep), #27 (schema-drift tripwire — Layer A + B). Structural fix shipped for #25 (TenneT 422 classifier + grid_imbalance soft-gate in 7673750); root-cause investigation of WHY 422 fires remains. **Still open**: #2 (JAO — needs API research), #9 (storage migration — defer), #21 (Liander watcher — low ROI), #25 (TenneT 422 root cause — structural noise removed, diagnosis remains), #28 (range-bound mismatches on solar/load surfaced by `validate_value_ranges` fix), #29 (per-zone completeness signal for hydro collector).
