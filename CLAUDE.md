...
| About to act on "it's probably transient" / "that's probably safe" | `memory/hypothesis-log.md` — open positions (H2, H4, H6, H8, H10, H11 as of 2026-09-21; H1/H3/H5/H7/H9 resolved) with the method that would settle each. Check before treating a recurring failure as known-benign; `/curate` surfaces overdue entries. |
...
collectors/
  ...
  _host_breaker.py           # PROCESS-WIDE circuit breaker keyed by HOST, shared across
                             ...
                             # none ever reached the threshold (2026-08-31: ~295 requests into
                             # an API 503-ing every one). Counts EXHAUSTED sub-requests,
                             # not HTTP attempts; any success closes it; empty-window and
                             # permanent-4xx errors never count (a healthy host with unpublished
                             # cable borders would otherwise trip it). One probe per 60s cooldown.
                             # Terminal-for-the-run for the six zone/border collectors;
                             # the two price collectors consult it inline and DO get 3
                             # rounds 300s apart, so the critical feed keeps its recovery.
  ...
  _entsoe_shared.py          # Per-zone delivery tracking for the four country-keyed
                             ...
                             # fetch and still vanish from `data` (added 2026-08-30 after the
                             # 08-29 ENTSO-E outage published an envelope claiming NL
                             # while `data` carried only DE_LU).
...
utils/
  ...
  data_quality.py            # FMEA validation. DATASET_MISSING_SEVERITY (single registry),
                             # PRESENT_EMPTY_GRACE_FEEDS (#42, the feeds eligible for the
                             # time-boxed present-empty grace — Open-Meteo plus
                             # ned_production; keyed on the failure mode, not the vendor),
                             # and present_empty_feeds(), its predicate,
...
.claude/                     # Agent harness config (committed). `review-changes`, `curate`,
                             # `audit-context` + `update-drift` are all user-global and NOT here;
                             # only `/release` is project-local. What IS here for review is
                             # `review-profile.md`, the project half of `/review-changes`.
  ...
  review-profile.md          # This repo's half of /review-changes — tiers, guarantee surfaces,
                             # test baseline, two project lenses. The skill is user-global;
                             # only this file is project-specific. Rows reviewed against the
                             # installed skill 2026-09-21.
...
.github/
  ...
  workflows/
    collect-data.yml         # Daily 16:00 UTC collection + publish. Includes completeness
                             ...
                             # Actions SHA-pinned (Node-24 since 2026-06-14). Owns the Pages deploy
                             # since 2026-07-06: uploads docs/, then a `deploy` job runs
                             # actions/deploy-pages with a 3-attempt retry (source =
                             # "GitHub Actions"; pages-build-deployment no longer runs).
...
| `scripts/detect_schema_drift.py` | CI tripwire — diffs `data/_shape_signatures.json` against `git show HEAD:`. Data-volatile feeds (declared + history-derived) warn instead of failing. Volatility is derived from `data/_shape_observations.jsonl` (#43), falling back to the sidecar's git history when that log has <2 records. **Member drift** (a location dropping out of a feed declared in `MEMBER_MAPPED_FEEDS`, all members sharing one shape) also warns — see `classify_data_member_drift`. Classified before volatility; `MEMBER_MAPPED_FEEDS` are excluded from derived volatility; `CRITICAL_FEEDS` never downgrade. **A diff that is only `metadata.collector_quality_issues` arriving/leaving is not drift** (#45): `_partition_diagnostic_only` prunes it on both sides and drops the feed — no `CRITICAL_FEEDS` carve-out, since nothing is downgraded. A real change underneath still fails. |
| `data/_shape_observations.jsonl` | Append-only learning record — one compact line per run (feed → shape_hash + schema_version). Written every run, committed *before* the drift gate. Never diff against it; it is history, not a baseline. Since 2026-09-21 records also carry `pruned_hashes` + `feeds_pruned`, so `derive_volatile_feeds()` judges on a **diagnostic-blind** hash and the drift gate's exemption cannot promote a feed to volatile. Per-feed; a mixed window falls back to raw hashes (inert until the window rolls). |
...
| `.claude/skills/` | `/release` only — project-local by design. `review-changes`, `curate`, `audit-context` + `update-drift` are user-global |
| `.claude/review-profile.md` | This repo's half of `/review-changes`: risk tiers, guarantee surfaces, test baseline, project lenses. Edit this, not the skill |
